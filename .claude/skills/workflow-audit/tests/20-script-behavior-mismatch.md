...
Verify that `workflow-audit` properly detects, classifies, and reports a mismatch between a workflow script's documented behavior and its actual behavior, including documented output-value contracts (e.g., `detect-embed-state.py` status values) and required environment-variable/human-confirmation contracts (e.g., `WORKFLOW_EMBED_HUMAN_CONFIRMED` plus human-terminal confirmation for `install-workflow.py`).

## Input

User input:

> Audit `docs/workflows/新项目开发工作流/`. Pay special attention to whether `install-workflow.py` still refuses formal install when `WORKFLOW_EMBED_HUMAN_CONFIRMED` is unset or human-terminal confirmation is missing, and whether `detect-embed-state.py` exit codes still match what the embed flow documentation promises.

Interpreted as:
```yaml
workflow_path: docs/workflows/新项目开发工作流/
candidate_issues:
  - Whether install-workflow.py still enforces WORKFLOW_EMBED_HUMAN_CONFIRMED and human-terminal confirmation for formal install
...
- in Step 2C, compare the documented contract (from workflow docs and from `references/human-terminal-required-template.md`) against actual script behavior; note that `detect-embed-state.py` always exits with code 0 and reports status via stdout (both human-readable and JSON), not via exit codes — the documented contract references status values (`INITIAL_BASELINE_READY`, `ALREADY_VALID_EMBEDDED`, `BLOCKED_NON_INITIAL_STATE`) which appear in the output, not in exit codes
- if the script no longer enforces `WORKFLOW_EMBED_HUMAN_CONFIRMED=1` and the explicit human-terminal confirmation boundary for formal install, classify it as a `P0` confirmed issue (security/boundary contract broken) per the priority rubric
...
- every confirmed issue includes the full minimum schema, with `validation action` describing exactly how the mismatch was detected (e.g., "Read `install-workflow.py:<line>` and confirmed the env-var or terminal confirmation check has been removed; documentation in `human-terminal-required-template.md` still implies enforcement")
...
- must not silently downgrade a broken `WORKFLOW_EMBED_HUMAN_CONFIRMED` / human-terminal confirmation enforcement to `P1` or `P2` — boundary-contract breakage is `P0`
...
- must not skip the comparison against `human-terminal-required-template.md` and the spec, since those are the documented contracts the script is being measured against
