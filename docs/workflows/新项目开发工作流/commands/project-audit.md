...
> **Workflow Position**: §5.1 → 前: 全部 `任务域=代码相关` 的任务完成 → 后: `/trellis:check` → `/trellis:review-gate` → `/trellis:finish-work` → `/trellis:delivery`
...
- `task_plan.md` 存在
- 任务执行矩阵中所有 `任务域=代码相关` 的任务均为 `已完成`
- `任务域=项目级审查` 的 `PROJECT-AUDIT` 任务尚未完成
...
| 审查发现冻结后新增 / 修改 / 删除需求 | `§2.5 需求变更管理` | 同上 | 先完成变更评估，不直接混入本轮审查修改 |
