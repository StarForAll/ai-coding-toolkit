...
> 若 `PRD` 已冻结后，用户在本阶段提出新增 / 修改 / 删除需求，先执行 `§2.5 需求变更管理`；不要直接改测试套件去吸收新的范围。只有纯澄清且不改变范围、接口契约、验收标准、成本、工期时，才留在本阶段处理。
...
| 冻结后出现新增 / 修改 / 删除需求 | `§2.5 需求变更管理` | 先完成变更评估、审批与基线更新，再回到受影响的最早阶段重建测试门禁 |
