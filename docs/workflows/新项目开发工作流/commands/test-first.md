...
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:test-first`） · ✅ Cursor（命令名: test-first） · ✅ OpenCode（TUI: `/trellis:test-first`；CLI: `trellis/test-first`；见 `opencode/README.md`） · ⚠️ Codex（同项目会安装 `test-first` skill；通过自然语言或显式 skill 触发，不提供项目级 `/trellis:test-first` 命令；见 `codex/README.md`） · ⚠️ Gemini（兼容层；见 `gemini/README.md`）
...
| 场景 | 调用能力 | 说明 |
|------|---------|------|
| 测试框架 API 查询 | `Context7` | 获取最新测试框架文档 |
| 复杂测试逻辑推理 | `sequential-thinking` | 多步推理、边界探索 |
