...
|---------|---------------------------|----------------|------|
| 基本合规，可直接进入收尾 | `/trellis:finish-work` | 进入提交前检查，或显式触发 `finish-work` skill | **默认推荐**（Lite / Standard）。检查通过可直接收尾 |
| 涉及安全 / 跨层 / 高 blast radius / 多 CLI 任务 | `/trellis:review-gate` | 进入补充审查判断，或显式触发 `review-gate` skill | **条件触发**。当 check 结果涉及高风险面时，由 review-gate 做门禁判定 |
