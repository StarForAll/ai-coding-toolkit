...
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:check`） · ✅ Cursor（命令名: check） · ✅ OpenCode（TUI: `/trellis:check`；CLI: `trellis/check`；见 `opencode/README.md`） · ⚠️ Codex（同项目会安装 `check` skill；通过自然语言或显式 skill 触发，不提供项目级 `/trellis:check` 命令；见 `codex/README.md`） · ⚠️ Gemini（兼容层；见 `gemini/README.md`）
...
| 场景 | 调用能力 | 说明 |
|------|---------|------|
| 复杂影响面推理 | `sequential-thinking` | blast radius 分析、多条件分支判定 |
| 依赖安全公告检查 | `exa_search` | 改动涉及依赖升级时查已知漏洞 |
