...
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:check`） · ✅ Cursor（命令名: check） · ✅ OpenCode（TUI: `/trellis:check`；CLI: `trellis/check`；见 `opencode/README.md`） · ⚠️ Codex（通过 AGENTS.md NL 路由触发，不提供项目级 `/trellis:check` 命令；见 `codex/README.md`） · ⚠️ Gemini（兼容层；见 `gemini/README.md`）
...
| 场景 | 调用能力 | 调用级别 | 说明 |
|------|---------|---------|------|
| 复杂影响面推理 | `sequential-thinking` | 按需 | blast radius 分析、多条件分支判定；当影响面涉及 ≥3 层或 ≥3 个高风险条件时触发 |
| 依赖安全公告检查 | `exa_search` | 按需 | 仅在改动涉及依赖升级或外部组件风险时触发。若无法联网，标记 `[Evidence Gap]`，不要给出“无已知漏洞”的结论 |
