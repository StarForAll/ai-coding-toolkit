...
这几个前提成立后，正确顺序是：先完成 `trellis init`，再执行当前 workflow 自带的安装脚本，由安装脚本把 Claude 所需命令入口和补丁真正落到目标项目，并自动导入 `pack.requirements-discovery-foundation`；若目标项目存在 `00-bootstrap-guidelines`，安装脚本会一并清理，否则跳过。
