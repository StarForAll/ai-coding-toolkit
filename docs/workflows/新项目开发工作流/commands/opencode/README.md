...
3. 上述 formal embed 必须由人类操作者在交互式系统终端中显式运行并确认；OpenCode 主会话本身不是被允许的首次嵌入执行主体
4. 安装完成后，在目标项目内直接使用 `/trellis:<phase>` 或 `trellis/<phase>`；初始 spec 基线由安装脚本完成，不再手工复制
...
- 当前嵌入 workflow 对 OpenCode **显式禁用** `agents / subagents` 执行路径
- 正式入口仍是 `.opencode/commands/trellis/*.md` 与主会话命令流转
- 即使目标项目仍存在 `trellis-research / trellis-implement / trellis-check` carrier，它们也只视为被当前 workflow 禁用的底层承载面
- `inject-subagent-context.js` 在当前 workflow 下应始终向 prompt 注入显式阻断说明，把控制权交回主会话，而不是静默继续
...
- **subagent hook 注入层**：需额外验证，不应默认与 Claude 等价；当前 workflow 通过 source-side patch 额外约束：
  - `inject-subagent-context.js` 对当前嵌入 workflow 统一阻断 `implement / check / research` 子代理路径
  - 插件会向 prompt 注入显式阻断说明，把控制权交回主会话，而不是只写 debug log
