...
其中 `brainstorm` 阶段命令还需要负责明确一条强制门禁：在进入 design 前，目标项目至少必须已经补齐 `docs/requirements/customer-facing-prd.md`；`docs/requirements/developer-facing-prd.md` 等到技术架构确认后再正式生成。
