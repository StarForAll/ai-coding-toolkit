...
|--------|---------|--------|
| `entry_choice_required` | No active task exists and nothing is resumable | Choose by intent first. For read-only workflow/project analysis, meta-audit, or A/A+ analysis, stay in `no_task` and analyze inline without creating a task. For a new implementation task, enter the formal route target first. If no reusable assessment exists, the outsourcing profile should enter `feasibility`, while a personal-profile first entry may enter `brainstorm` directly and bootstrap the minimum assessment baseline needed to stay in `brainstorm` and leave it safely. That minimum baseline does not replace the fuller feasibility assessment when later decisions require it. Outside that personal bootstrap branch, `brainstorm` is only valid when route explicitly reuses an existing assessment that already allows it. If route also reports `profile_hint=unknown`, keep the conservative `feasibility` fallback and ask the user to confirm whether the project should be treated as outsourcing or personal before skipping feasibility; do not guess. Do not assume a public `implementation` skill exists. |
| `profile_confirmation_required` | Project type cannot yet be trusted from assessment or install metadata | Ask the user whether this project should be treated as `outsourcing` or `personal` before continuing. Do not silently fall back to feasibility/brainstorm, and do not guess from repo name, tone, or partial docs. |
...
| `repair_needed` | State file missing or corrupt | Run `workflow-state.py repair` without `--apply` first and inspect its `status`, `evidence`, and `blockers`. If it reports `repair_ready`, confirm with the user and rerun with `--apply`. If it reports `manual_confirmation_required`, ask the user to confirm the currently approved stage and any missing execution-boundary details, then rerun with explicit confirmation flags instead of inferring from artifacts. Non-execution stages usually only need `--stage <stage>`; `implementation` / `test-first` also require `--execution-authorized true` and `--transition-from <previous-stage>`. After apply succeeds, rerun `route` or `validate` before resuming the stage body. |
