...
|--------|---------|--------|
| `entry_choice_required` | No active task exists and nothing is resumable | Choose by intent first. For read-only workflow/project analysis, meta-audit, or A/A+ analysis, stay in `no_task` and analyze inline without creating a task. For a new implementation task, enter the formal route target first. If no reusable assessment exists, the outsourcing profile should enter `feasibility`, while a personal-profile first entry may enter `brainstorm` directly and bootstrap the minimum assessment baseline there before leaving the stage. Outside that personal bootstrap branch, `brainstorm` is only valid when route explicitly reuses an existing assessment that already allows it. If route also reports `profile_hint=unknown`, keep the conservative `feasibility` fallback and ask the user to confirm whether the project should be treated as outsourcing or personal before skipping feasibility; do not guess. Do not assume a public `implementation` skill exists. |
