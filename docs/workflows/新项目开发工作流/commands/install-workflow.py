...
def _resolve_trellis_scripts_dir() -> Path:
    env_root = os.environ.get(_SOURCE_REPO_ROOT_ENV)
    ...
    for candidate in candidates:
        ...
        while current != current.parent:
            trellis_scripts = current / ".trellis" / "scripts"
            if (trellis_scripts / "common" / "active_task.py").is_file():
                return trellis_scripts
            current = current.parent

    fallback = Path(__file__).resolve().parents[4] / ".trellis" / "scripts"
    if (fallback / "common" / "active_task.py").is_file():
        return fallback
    raise RuntimeError("Unable to resolve host .trellis/scripts directory")
    ...
    *,
    ...
    if not dry_run:
        rec.write_text(json.dumps({
            ...
            "patched_codex_skills": CODEX_PATCH_BASELINE_SKILLS if "codex" in cli_types else [],
