...
def source_repo_root() -> Path:
    ...
    env_root = os.environ.get(SOURCE_REPO_ROOT_ENV)
    if env_root:
        candidate = Path(env_root).expanduser().resolve()
        if (candidate / ".trellis").is_dir() and (candidate / "trellis-library" / "cli.py").is_file():
            return candidate
    ...
    for candidate in candidates:
        ...
        while current != current.parent:
            if (current / ".trellis").is_dir() and (current / "trellis-library" / "cli.py").is_file():
                return current
            current = current.parent

    fallback = Path(__file__).resolve().parents[4]
    if (fallback / ".trellis").is_dir() and (fallback / "trellis-library" / "cli.py").is_file():
        return fallback
    raise RuntimeError("Unable to resolve workflow source repository root")
