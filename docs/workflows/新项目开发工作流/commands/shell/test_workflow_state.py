...
"""
    ...
    def write_workflow_state(
        ...
        *,
        ...
        architecture_confirmed: bool = False,
        context7_review_completed: bool = False,
        ...
        state: dict[str, object] = {
            ...
            "checkpoints": {
                ...
                "context7_review_completed": context7_review_completed,
        ...
        if transition_from is not None:
            state["last_confirmed_transition"] = {
                ...
                "to": stage,
                "confirmed_at": "2026-04-16T00:00:00+00:00",
            }
    ...
    def write_project_audit_report(
        self,
        task_dir: Path,
        content: str | None = None,
        *,
        augment_contract: bool = True,
    ) -> None:
        report_content = (
            content
...
- `task_level_check_task`: `current_active_task`
...
"""
        ...
        if augment_contract and "`task_level_check_status`" in report_content and "`task_level_check_task`" not in report_content:
            report_content = report_content.replace(
                "`task_level_check_status`: `pass`",
                "`task_level_check_status`: `pass`\n- `task_level_check_task`: `current_active_task`",
            ).replace(
                "`task_level_check_status`: `fail`",
                "`task_level_check_status`: `fail`\n- `task_level_check_task`: `current_active_task`",
            ).replace(
                "`task_level_check_status`: `not_needed`",
                "`task_level_check_status`: `not_needed`\n- `task_level_check_task`: `current_active_task`",
            ).replace(
                "`task_level_check_status`: `not_run`",
                "`task_level_check_status`: `not_run`\n- `task_level_check_task`: `current_active_task`",
            )
    ...
    def write_task_plan_with_code_tasks(
        ...
        *,
        ...
        # Helper note: this writes a structured `project-audit` row for the
        # current task itself. Tests that need "formal PROJECT-AUDIT declared on
        # another task" must write task_plan.md manually instead of using this
        # helper, otherwise the current task becomes the carrier by construction.
        tasks_root = root / ".trellis" / "tasks"
        rows: list[str] = [
            "# Task Plan",
            "",
        ...
        for task_name, status in statuses.items():
            ...
            task_type = (task_types or {}).get(task_name, "implementation")
            rows.append(
                f"| .trellis/tasks/{task_name} | {task_type} | 全局 | status={status} |"
            )
...
""",
    ...
    def write_delivery_artifacts(
        ...
        *,
        ...
        if valid_contract:
            ...
            (delivery_dir / "retrospective.md").write_text(
                "# Retrospective\n\n"
                "## 本轮验收\n"
                "- pass\n\n"
                "## 返工\n"
            ...
            (delivery_dir / "retrospective.md").write_text("# retrospective\n", encoding="utf-8")
    ...
    def test_plan_to_implementation_leaf_inherits_parent_plan_artifacts(self) -> None:
        root, parent_dir = self.make_fixture()
        child_dir = root / ".trellis" / "tasks" / "04-15-child-task"
        child_dir.mkdir(parents=True, exist_ok=True)
        (parent_dir / "task.json").write_text(
            '{"status":"planning","children":["04-15-child-task"]}\n',
            encoding="utf-8",
        )
        ...
        (root / ".trellis" / ".runtime" / "sessions" / "test-context.json").write_text(
            json.dumps({"current_task": ".trellis/tasks/04-15-child-task"}, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        self.write_required_project_docs(
            root,
        ...
        self.write_context7_review(parent_dir)
        performance_task_dir = root / ".trellis" / "tasks" / "04-15-performance-opt"
        performance_task_dir.mkdir(parents=True, exist_ok=True)
        (performance_task_dir / "task.json").write_text('{"status":"planning","children":[]}\n', encoding="utf-8")
        self.write_ownership_plan_task_dirs(root)
        ...
        (parent_dir / "task_plan.md").write_text(child_plan, encoding="utf-8")
        (parent_dir / "task_creation_checklist.md").write_text(self.VALID_TASK_CREATION_CHECKLIST, encoding="utf-8")
        design_dir = parent_dir / "design"
        design_dir.mkdir(parents=True, exist_ok=True)
        ...
        (child_dir / "prd.md").write_text(
            "# Leaf Task\n\n"
            "## Goal\n\n验证 leaf 继承 parent plan 产物。\n\n"
            "## In Scope\n\n- 校验从 parent plan 合法进入 implementation。\n\n"
            "## Out of Scope\n\n- 不进入实际实现。\n\n"
            "## Acceptance Anchors\n\n- 阶段切换命令通过。\n\n"
            "## Preferred CLI\n\n- Codex\n",
            encoding="utf-8",
        ...
        self.run_script("init", str(child_dir), "--stage", "plan")
        self.run_script("set", str(child_dir), "--context7-review-completed", "true")
        self.run_script(
            "set",
            str(child_dir),
    ...
    def test_validate_project_audit_formal_blocks_when_code_related_tasks_are_incomplete(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        self.write_task_plan_with_code_tasks(
            ...
            statuses={
                "04-15-sample-task": "completed",
                "04-15-performance-opt": "in_progress",
            },
        )
        self.run_script("init", str(task_dir), "--stage", "project-audit")
...
""",
    ...
    def test_validate_project_audit_accepts_structured_project_audit_mode_field(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
    ...
    def test_route_design_exit_blocks_when_ui_lane_missing_external_prototype_evidence(self) -> None:
        root, task_dir = self.make_fixture()
        requirements_dir = root / "docs" / "requirements"
        self.write_required_project_docs(
            root,
        ...
        blockers_text = "".join(route_data.get("blockers", []))
        self.assertIn("STITCH-PROMPT.md", blockers_text)
        self.assertIn("结构化", blockers_text)

    def test_route_design_exit_allows_ui_lane_when_external_prototype_evidence_exists(self) -> None:
        root, task_dir = self.make_fixture()
        requirements_dir = root / "docs" / "requirements"
        self.write_required_project_docs(
            root,
        ...
        design_dir = task_dir / "design"
        design_dir.mkdir(parents=True, exist_ok=True)
        (design_dir / "STITCH-PROMPT.md").write_text(
            "# STITCH-PROMPT\n"
        ...
        route = self.run_script("route", str(task_dir), "--project-root", str(root))
        route_data = json.loads(route.stdout)
        self.assertEqual(route_data["action"], "awaiting_confirmation")
...
## Project-Level Verification Matrix
- `project-task-coverage`: pre-audit coverage only
- 项目级统一代码漏洞检测命令：not run + reason
- 项目级统一代码质量总检命令：not run + reason
...
""",
    ...
    def test_pre_audit_cannot_exit_directly_to_delivery(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
## Project-Level Verification Matrix
- `project-task-coverage`: partial pre-audit only
- 项目级统一代码漏洞检测命令：not run + reason
- 项目级统一代码质量总检命令：not run + reason
...
""",
        ...
        self.assertIn("pre-audit", blocked.stdout)
...
""",
    ...
    def test_validate_review_gate_recommended_lite_requires_real_reviewer_report(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
        ...
        validate = self.run_script("validate", str(task_dir), "--project-root", str(root))
        self.assertEqual(validate.returncode, 1, msg=validate.stdout + validate.stderr)
        self.assertIn("reviewer 报告", validate.stdout)
    ...
    def test_validate_delivery_rejects_invalid_retrospective_contract_when_present(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        self.write_delivery_artifacts(task_dir, valid_contract=True, include_outsourcing_proofs=True)
        (task_dir / "delivery" / "retrospective.md").write_text("# retrospective\n", encoding="utf-8")
        self.write_finish_work_checklist(task_dir)
        self.run_script("init", str(task_dir), "--stage", "delivery")
        validate = self.run_script("validate", str(task_dir), "--project-root", str(root))
        self.assertEqual(validate.returncode, 1, msg=validate.stdout + validate.stderr)
        self.assertIn("retrospective.md", validate.stdout)

    def test_validate_delivery_blocks_without_delivery_and_finish_work_gate_status(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        self.write_delivery_artifacts(task_dir, valid_contract=True, include_outsourcing_proofs=True)
        (task_dir / "delivery" / "retrospective.md").unlink()
        self.write_finish_work_checklist(task_dir)
        acceptance_path = task_dir / "delivery" / "acceptance.md"
        acceptance_path.write_text(
            acceptance_path.read_text(encoding="utf-8").replace("- `delivery_gate_status`: `pass`\n", ""),
            encoding="utf-8",
        )
        checklist_path = task_dir / "finish-work-checklist.md"
        checklist_path.write_text(
            checklist_path.read_text(encoding="utf-8").replace("- `finish_work_gate_status`: `pass`\n", ""),
            encoding="utf-8",
        )
        self.run_script("init", str(task_dir), "--stage", "delivery")
        validate = self.run_script("validate", str(task_dir), "--project-root", str(root))
        self.assertEqual(validate.returncode, 1, msg=validate.stdout + validate.stderr)
        self.assertTrue(
            "delivery_gate_status" in validate.stdout or "finish_work_gate_status" in validate.stdout
        )
    ...
    def test_init_rejects_direct_execution_stage_state(self) -> None:
        _root, task_dir = self.make_fixture()

        result = self.run_script("init", str(task_dir), "--stage", "implementation")

        self.assertEqual(result.returncode, 1, msg=result.stdout + result.stderr)
        self.assertIn("init 不允许直接创建执行阶段状态", result.stdout)
        self.assertFalse((task_dir / "workflow-state.json").exists())
    ...
    def test_validate_fails_when_implementation_has_no_execution_authorization(self) -> None:
        ...
        self.write_workflow_state(task_dir, stage="implementation")
        validate = self.run_script("validate", str(task_dir), "--project-root", str(root))
    ...
    def test_set_awaiting_confirmation_does_not_persist_redundant_helper_field(self) -> None:
        _root, task_dir = self.make_fixture()
        self.run_script("init", str(task_dir), "--stage", "plan")
        ...
        self.assertNotIn("awaiting_user_confirmation", state)

    def test_set_awaiting_confirmation_alias_updates_status_without_explicit_stage_status(self) -> None:
        _root, task_dir = self.make_fixture()
        self.run_script("init", str(task_dir), "--stage", "plan")
    ...
    def test_set_rejects_conflicting_awaiting_confirmation_alias_and_stage_status(self) -> None:
        _root, task_dir = self.make_fixture()
        self.run_script("init", str(task_dir), "--stage", "plan")
        ...
        self.assertEqual(result.returncode, 1, msg=result.stdout + result.stderr)
        self.assertIn("--awaiting-user-confirmation", result.stdout)
    ...
    def test_set_rejects_plan_to_implementation_without_execution_authorization(self) -> None:
        ...
        self.assertEqual(illegal_set.returncode, 1, msg=illegal_set.stdout + illegal_set.stderr)
        self.assertTrue(
            "status 必须为 awaiting_user_confirmation" in illegal_set.stdout
            or "缺少 --transition-from" in illegal_set.stdout,
            msg=illegal_set.stdout + illegal_set.stderr,
        )
        ...
        self.assertTrue(
            "execution_authorized 必须为 true" in illegal_set2.stdout
            or "缺少 --transition-from" in illegal_set2.stdout,
            msg=illegal_set2.stdout + illegal_set2.stderr,
        )
    ...
    def test_route_execution_stage_allows_project_estimate_from_parent_task_prd(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        result = self.run_script("route", str(child_dir), "--project-root", str(root))

        data = json.loads(result.stdout)
        self.assertEqual(data["action"], "reenter", msg=result.stdout + result.stderr)

    def test_validate_execution_stage_allows_project_estimate_from_parent_task_prd(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        validate = self.run_script("validate", str(child_dir), "--project-root", str(root))
    ...
    def test_route_check_awaiting_allows_project_level_targets_when_formal_project_audit_is_not_declared(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        self.run_script(
            ...
            "--stage-status", "awaiting_user_confirmation",
            "--awaiting-user-confirmation", "true",
            "--allowed-next", "project-audit,delivery",
        )
    ...
    def test_route_check_awaiting_blocks_when_formal_project_audit_is_declared_on_other_task(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
    ...
    def test_check_to_project_audit_allows_when_current_task_is_formal_project_audit_carrier(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
    ...
    def test_check_to_project_audit_blocks_when_formal_project_audit_is_declared_on_other_task(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        self.assertTrue(
            ...
            or "formal project-audit 缺少 task_plan.md" in blocked.stdout
            or "至少需要一个候选 task 满足正式门禁" in blocked.stdout,
            msg=blocked.stdout + blocked.stderr,
        )

    def test_review_gate_stage_cannot_switch_to_project_audit_even_when_allowed_next_mentions_it(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
    ...
    def test_project_audit_to_delivery_missing_check_report_mentions_delivery(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        self.assertEqual(blocked.returncode, 1, msg=blocked.stdout + blocked.stderr)
        self.assertIn("不得进入 delivery", blocked.stdout)
        self.assertNotIn("不得进入 review-gate", blocked.stdout)
...
""",
        ...
        self.assertEqual(blocked.returncode, 1, msg=blocked.stdout + blocked.stderr)
        self.assertIn("不得直接进入 delivery", blocked.stdout)

    def test_project_audit_to_delivery_blocks_when_project_audit_gate_status_is_fail(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
- 项目级统一代码漏洞检测：not run + reason
- 项目级统一代码质量总检：not run + reason
- `project_audit_gate_status`: `fail`
- `task_level_check_status`: `pass`
...
""",
        ...
        self.assertEqual(blocked.returncode, 1, msg=blocked.stdout + blocked.stderr)
        self.assertIn("project_audit_gate_status", blocked.stdout)

    def test_project_audit_to_delivery_blocks_when_project_audit_gate_status_is_not_run(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
- 项目级统一代码漏洞检测：not run + reason
- 项目级统一代码质量总检：not run + reason
- `project_audit_gate_status`: `not_run`
- `task_level_check_status`: `pass`
...
""",
        ...
        self.assertEqual(blocked.returncode, 1, msg=blocked.stdout + blocked.stderr)
        self.assertIn("project_audit_gate_status", blocked.stdout)
        self.assertIn("兼容", blocked.stdout)

    def test_project_audit_stage_cannot_switch_to_review_gate_even_when_gate_status_is_missing(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
    ...
    def test_project_audit_stage_cannot_switch_to_review_gate_even_when_gate_status_is_fail(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
## Project-Level Verification Matrix
- `project-task-coverage`: all code-related tasks complete; blockers remain
- 项目级统一代码漏洞检测命令：not run + reason
- 项目级统一代码质量总检命令：not run + reason

## Confirmed Findings
- [self] blockers remain
...
""",
    ...
    def test_project_audit_to_implementation_is_not_a_canonical_transition(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
## Project-Level Verification Matrix
- `project-task-coverage`: all code-related tasks complete; remediation required before close-out
- 项目级统一代码漏洞检测命令：not run + reason
- 项目级统一代码质量总检命令：not run + reason

## Confirmed Findings
- [self] implementation rework required

## Candidate Findings / Reviewer Evidence
- [self] none

## Confirmed Fix Plan
- return to implementation for targeted rework
...
## Suggested Next Step
- 回到 implementation 继续实作，修复发现的架构问题
""",
        )
        self.run_script("init", str(task_dir), "--stage", "project-audit")
        self.run_script(
            ...
            "--stage-status", "awaiting_user_confirmation",
            "--awaiting-user-confirmation", "true",
            "--allowed-next", "check,review-gate,implementation,delivery",
        )
        blocked = self.run_script(
            "set",
            str(task_dir),
    ...
    def test_project_audit_to_delivery_blocks_without_project_audit_gate_status(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
    ...
    def test_project_audit_to_delivery_allows_when_current_task_is_formal_project_audit_carrier(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
    ...
    def test_project_audit_to_delivery_blocks_without_task_plan_in_formal_mode(self) -> None:
        ...
        self.assertEqual(blocked.returncode, 1, msg=blocked.stdout + blocked.stderr)
        self.assertIn("formal project-audit 缺少 task_plan.md", blocked.stdout)

    def test_project_audit_stage_cannot_switch_to_review_gate_even_when_code_changes_exist(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
    ...
    def test_delivery_blocks_when_formal_project_audit_task_is_not_completed(self) -> None:
        ...
        self.write_check_report(task_dir)
        sample_task_dir = root / ".trellis" / "tasks" / "04-15-sample-task"
        sample_task_dir.mkdir(parents=True, exist_ok=True)
        (sample_task_dir / "task.json").write_text(
            json.dumps({"id": "04-15-sample-task", "name": "04-15-sample-task", "status": "completed"}, ensure_ascii=False)
            + "\n",
            encoding="utf-8",
        )
        (task_dir / "task_plan.md").write_text(
        ...
        formal_dir.mkdir(parents=True, exist_ok=True)
        self.write_project_audit_report(formal_dir)
        self.write_delivery_artifacts(formal_dir, valid_contract=True)
        self.write_finish_work_checklist(formal_dir)
        self.write_delivery_artifacts(task_dir, valid_contract=True)
        self.write_finish_work_checklist(task_dir)
        self.run_script("init", str(task_dir), "--stage", "delivery")
        self.run_script(
        ...
        blocked = self.run_script("validate", str(task_dir), "--project-root", str(root))
        self.assertEqual(blocked.returncode, 1, msg=blocked.stdout + blocked.stderr)
        self.assertTrue(
            "formal PROJECT-AUDIT task 尚未完成" in blocked.stdout
            or "至少需要一个候选 task 满足正式门禁" in blocked.stdout,
            msg=blocked.stdout + blocked.stderr,
        )

    def test_delivery_blocks_when_formal_project_audit_task_lacks_gate_evidence(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
        ...
        self.assertEqual(blocked.returncode, 1, msg=blocked.stdout + blocked.stderr)
        self.assertTrue(
            "project_audit_gate_status" in blocked.stdout
            or "至少需要一个候选 task 满足正式门禁" in blocked.stdout,
            msg=blocked.stdout + blocked.stderr,
        )

    def test_delivery_uses_parent_check_context_for_project_audit_carrier_task(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        (formal_dir / "task.json").write_text(
            json.dumps(
                {
                    "id": "04-15-project-audit-formal",
                    "name": "04-15-project-audit-formal",
                    "status": "in_progress",
                    "parent": task_dir.name,
                },
                ensure_ascii=False,
        ...
        (formal_dir / "prd.md").write_text(
            "# formal project audit\n\n"
            "## Goal\n\n项目级复核。\n\n"
            "## In Scope\n\n- 项目级审查。\n\n"
            "## Out of Scope\n\n- 不扩展范围。\n\n"
            "## Acceptance Anchors\n\n- 审查门禁通过。\n\n"
...
""",
        ...
        self.write_delivery_artifacts(formal_dir, valid_contract=True, include_outsourcing_proofs=True)
        self.write_finish_work_checklist(formal_dir)
        self.run_script("init", str(formal_dir), "--stage", "delivery")

        validate = self.run_script("validate", str(formal_dir), "--project-root", str(root))

        self.assertEqual(validate.returncode, 0, msg=validate.stdout + validate.stderr)

    def test_delivery_uses_explicit_task_level_check_task_for_project_audit_carrier_task(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        self.write_check_report(task_dir)
        (task_dir / "task.json").write_text(
            json.dumps({"id": task_dir.name, "name": task_dir.name, "status": "completed"}, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        formal_dir = root / ".trellis" / "tasks" / "04-15-project-audit-formal"
        formal_dir.mkdir(parents=True, exist_ok=True)
        (formal_dir / "task.json").write_text(
            json.dumps({"id": "04-15-project-audit-formal", "name": "04-15-project-audit-formal", "status": "in_progress"}, ensure_ascii=False)
            + "\n",
            encoding="utf-8",
        )
        self.write_required_project_docs(
...
- `project_audit_gate_status`: `pass`
- `task_level_check_status`: `pass`
- `task_level_check_task`: `.trellis/tasks/{task_dir.name}`
...
""",
    ...
    def test_delivery_ignores_unstructured_project_audit_mentions_without_formal_task_row(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        self.write_check_report(task_dir)
        self.write_delivery_artifacts(task_dir, valid_contract=True)
        (task_dir / "delivery" / "ownership-proof.md").write_text("# ownership proof\n", encoding="utf-8")
        (task_dir / "delivery" / "source-watermark-verification.md").write_text("# watermark verification\n", encoding="utf-8")
        self.write_finish_work_checklist(task_dir)
        (task_dir / "task_plan.md").write_text(
            "\n".join(
                [
    ...
    def test_delivery_blocks_when_formal_project_audit_task_declares_invalid_task_level_check_task(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
- `project_audit_gate_status`: `pass`
- `task_level_check_status`: `pass`
- `task_level_check_task`: `.trellis/tasks/does-not-exist`
...
""",
        ...
        self.assertEqual(validate.returncode, 1, msg=validate.stdout + validate.stderr)
        self.assertIn("task_level_check_task", validate.stdout)
        self.assertIn("无法解析到合法 task 目录", validate.stdout)

    def test_check_to_delivery_blocks_when_formal_project_audit_task_is_not_ready_at_transition(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        self.assertEqual(blocked.returncode, 1, msg=blocked.stdout + blocked.stderr)
        self.assertIn("formal PROJECT-AUDIT carrier", blocked.stdout)
    ...
    def test_delivery_reports_first_formal_project_audit_candidate_only(self) -> None:
        ...
        self.write_check_report(task_dir)
        tasks_root = root / ".trellis" / "tasks"
        first_formal = tasks_root / "04-15-project-audit-formal-a"
        second_formal = tasks_root / "04-15-project-audit-formal-b"
        first_formal.mkdir(parents=True, exist_ok=True)
        second_formal.mkdir(parents=True, exist_ok=True)
        (first_formal / "task.json").write_text(
            json.dumps({"id": "04-15-project-audit-formal-a", "name": "formal-a", "status": "completed"}, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        (second_formal / "task.json").write_text(
            json.dumps({"id": "04-15-project-audit-formal-b", "name": "formal-b", "status": "completed"}, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        self.write_project_audit_report(
            first_formal,
...
""",
        ...
        (task_dir / "task_plan.md").write_text(
            "\n".join(
                [
                    ...
                    "| 任务路径 | 类型 | 项目域 | 说明 |",
                    "|---------|------|--------|------|",
                    "| .trellis/tasks/04-15-sample-task | implementation | 全局 | completed |",
                    "| .trellis/tasks/04-15-project-audit-formal-a | project-audit | 全局 | formal candidate a |",
                    "| .trellis/tasks/04-15-project-audit-formal-b | project-audit | 全局 | formal candidate b |",
                ]
            ) + "\n",
            encoding="utf-8",
        )
        ...
        validate = self.run_script("validate", str(task_dir), "--project-root", str(root))
        self.assertEqual(validate.returncode, 1, msg=validate.stdout + validate.stderr)
        self.assertIn("至少需要一个候选 task 满足正式门禁", validate.stdout)
        self.assertEqual(validate.stdout.count("project_audit_gate_status"), 1)

    def test_delivery_blocks_when_formal_project_audit_check_owner_is_missing(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        formal_task_data["parent"] = task_dir.name
        (formal_dir / "task.json").write_text(
            json.dumps(formal_task_data, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        self.write_project_audit_report(
            formal_dir,
...
""",
    ...
    def test_validate_project_audit_blocks_frontend_type_without_explicit_coverage_when_task_incomplete(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
        ...
        self.write_project_audit_report(task_dir)
        tasks_root = root / ".trellis" / "tasks"
        frontend_task = tasks_root / "04-15-frontend-shell"
        frontend_task.mkdir(parents=True, exist_ok=True)
        (frontend_task / "task.json").write_text(
            json.dumps({"id": "04-15-frontend-shell", "name": "04-15-frontend-shell", "status": "planning"}, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        (task_dir / "task_plan.md").write_text(
            "\n".join(
                [
                    ...
                    "| 任务路径 | 类型 | 项目域 | 说明 |",
                    "|---------|------|--------|------|",
                    "| .trellis/tasks/04-15-frontend-shell | frontend | ui | landing shell implementation |",
                    f"| .trellis/tasks/{task_dir.name} | project-audit | 全局 | current task |",
                ]
            )
            + "\n",
    ...
    def test_validate_project_audit_blocks_when_gate_status_conflicts_with_fail_results(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
## Project-Level Verification Matrix
- `project-task-coverage`: covers 04-15-sample-task only; no approved exceptions; no delivery blockers
- 项目级统一代码漏洞检测命令：pass
- 项目级统一代码质量总检命令：pass

## Confirmed Findings
- [self] one blocker remained
...
""",
        ...
        self.assertEqual(validate.returncode, 1, msg=validate.stdout + validate.stderr)
        self.assertIn("project_audit_gate_status", validate.stdout)
...
""",
    ...
    def test_check_to_delivery_blocks_when_check_gate_status_is_fail(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
    ...
    def test_check_to_delivery_blocks_when_chinese_verification_results_contains_fail(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
    ...
    def test_validate_review_gate_allows_recommended_lite_not_run_for_acknowledged_capability_gap(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
## Trigger Evidence
- 需要额外视角，但本轮 reviewer CLI 未具备对应能力

## Mode
- `review_gate_mode`: `lite`
...
- `review_gate_capability_gap_reason`: `缺少 multi-cli-review / multi-cli-review-action 能力，用户已明确接受本轮残余风险`
""",
        )
        self.run_script("init", str(task_dir), "--stage", "review-gate")
    ...
    def test_review_gate_to_delivery_allows_when_formal_project_audit_is_not_declared(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
    ...
    def test_review_gate_to_delivery_blocks_when_formal_project_audit_is_declared_on_other_task(self) -> None:
        ...
        formal_dir.mkdir(parents=True, exist_ok=True)
        (formal_dir / "task.json").write_text(
            json.dumps({"id": "04-15-project-audit-formal", "name": "04-15-project-audit-formal", "status": "planning"}, ensure_ascii=False)
            + "\n",
            encoding="utf-8",
        )
        (task_dir / "task_plan.md").write_text(
    ...
    def test_review_gate_to_delivery_allows_when_current_task_is_formal_project_audit_carrier(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
        ...
        wrong_tmp = task_dir / "tmp" / "multi-cli-review" / task_dir.name / "review-round-1"
        wrong_tmp.mkdir(parents=True, exist_ok=True)
        (wrong_tmp / "a.md").write_text("# reviewer a\n", encoding="utf-8")
        (wrong_tmp / "b.md").write_text("# reviewer b\n", encoding="utf-8")

        self.run_script("init", str(task_dir), "--stage", "review-gate")
        blocked = self.run_script("validate", str(task_dir), "--project-root", str(root))
        self.assertEqual(blocked.returncode, 1, msg=blocked.stdout + blocked.stderr)
        self.assertIn("reviewer 报告不足 2 份", blocked.stdout)

        root_tmp = root / "tmp" / "multi-cli-review" / task_dir.name
        (root_tmp / "review-round-1").mkdir(parents=True, exist_ok=True)
        (root_tmp / "review-round-1" / "a.md").write_text("# reviewer a\n", encoding="utf-8")
        (root_tmp / "review-round-1" / "b.md").write_text("# reviewer b\n", encoding="utf-8")
        (root_tmp / "summary-round-1.md").write_text("- pass\n", encoding="utf-8")
        (root_tmp / "action.md").write_text("- adopted\n", encoding="utf-8")

        allowed = self.run_script("validate", str(task_dir), "--project-root", str(root))
        self.assertEqual(allowed.returncode, 0, msg=allowed.stdout + allowed.stderr)

    def test_validate_review_gate_recommended_full_requires_two_reviewer_reports(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
    ...
    def test_project_audit_stage_cannot_switch_to_review_gate_stage(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
## Project-Level Verification Matrix
- `project-task-coverage`: covered={task_dir.name}; exceptions=none; blockers=none
- 项目级统一代码漏洞检测命令：not run + reason
- 项目级统一代码质量总检命令：not run + reason
...
""",
        ...
        self.run_script("init", str(formal_dir), "--stage", "project-audit")
        self.run_script(
            "set",
            str(formal_dir),
            "--stage-status", "awaiting_user_confirmation",
            "--awaiting-user-confirmation", "true",
            "--allowed-next", "check,review-gate,delivery",
        )
    ...
    def test_route_project_audit_awaiting_requires_project_audit_gate_status(self) -> None:
        root, task_dir = self.make_fixture()
        self.write_required_project_docs(
            root,
            task_dir,
...
""",
        ...
        blocked_data = json.loads(blocked.stdout)
        self.assertEqual(blocked_data["action"], "awaiting_confirmation_with_blockers")
        self.assertIn("project_audit_gate_status", "".join(blocked_data.get("blockers", [])))
