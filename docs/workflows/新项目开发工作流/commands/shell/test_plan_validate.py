...
ALL_COMPLETED_PLAN = VALID_PLAN.replace("| TASK-A | 代码相关 | 无 | 可开始 |", "| TASK-A | 代码相关 | 无 | 已完成 |").replace(
    "| TASK-A2 | 代码相关 | 无 | 可开始 |", "| TASK-A2 | 代码相关 | 无 | 已完成 |"
).replace(
    "| TASK-B | 代码相关 | TASK-A | 等待中 | TASK-A 完成后可开始 | 等待 TASK-A 完成 |",
    "| TASK-B | 代码相关 | TASK-A | 已完成 | TASK-A 完成后可开始 | 已完成，无等待 |",
).replace(
    "| TASK-C | 代码相关 | TASK-B | 等待中 | TASK-B 完成后可开始 | 等待 TASK-B 完成 |",
    "| TASK-C | 代码相关 | TASK-B | 已完成 | TASK-B 完成后可开始 | 已完成，无等待 |",
).replace(
    "| PROJECT-AUDIT | 项目级审查 | TASK-A,TASK-B,TASK-C,TASK-A2 | 等待中 | 全部代码相关任务完成后可开始 | 等待全部代码相关任务完成 |",
    "| PROJECT-AUDIT | 项目级审查 | TASK-A,TASK-B,TASK-C,TASK-A2 | 已完成 | 全部代码相关任务完成后可开始 | 已完成，无等待 |",
).replace(
    "- 当前可开始任务：TASK-A, TASK-A2\n- 等待中任务：\n  - TASK-B（等待 TASK-A 完成）\n  - TASK-C（等待 TASK-B 完成）\n  - PROJECT-AUDIT（等待全部代码相关任务完成）\n- 推荐并行组：TASK-A + TASK-A2\n- 串行主链：TASK-A → TASK-B → TASK-C",
    "- 当前可开始任务：无\n- 等待中任务：无\n- 推荐并行组：无\n- 串行主链：TASK-A → TASK-B → TASK-C → PROJECT-AUDIT（已全部完成）",
)
...
class PlanValidateScriptTests(unittest.TestCase):
    ...
    def test_invalid_status_enum_fails(self) -> None:
        with tempfile.TemporaryDirectory() as temp_root:
            task_dir = Path(temp_root)
            content = VALID_PLAN.replace("| TASK-A | 代码相关 | 无 | 可开始 |", "| TASK-A | 代码相关 | 无 | ready |")
            self.write_plan(task_dir, content)
    ...
    def test_waiting_task_without_reason_fails(self) -> None:
        with tempfile.TemporaryDirectory() as temp_root:
            task_dir = Path(temp_root)
            content = VALID_PLAN.replace("| TASK-B | 代码相关 | TASK-A | 等待中 | TASK-A 完成后可开始 | 等待 TASK-A 完成 |", "| TASK-B | 代码相关 | TASK-A | 等待中 | TASK-A 完成后可开始 | 无 |")
            self.write_plan(task_dir, content)
    ...
    def test_non_waiting_task_with_empty_wait_reason_still_passes(self) -> None:
        with tempfile.TemporaryDirectory() as temp_root:
            ...
            content = VALID_PLAN.replace(
                "| TASK-A | 代码相关 | 无 | 可开始 | 需求、接口、环境、资源已就绪 | 无 | 候选可并行 | 当前可与 TASK-A2 并行，不可与其后续依赖任务并行 |",
                "| TASK-A | 代码相关 | 无 | 可开始 | 需求、接口、环境、资源已就绪 |  | 候选可并行 | 当前可与 TASK-A2 并行，不可与其后续依赖任务并行 |",
            )
            self.write_plan(task_dir, content)
