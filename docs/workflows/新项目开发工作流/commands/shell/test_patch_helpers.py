...
class PatchHelperScriptTests(unittest.TestCase):
    ...
    def test_patch_task_store_archive_guard_allows_check_stage_after_validate_passes(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="task-store-archive-check-stage-"))
        self.addCleanup(shutil.rmtree, root)
        package_dir = root / "pkg"
        package_dir.mkdir(parents=True, exist_ok=True)
        (package_dir / "__init__.py").write_text("", encoding="utf-8")
        ...
        (task_dir / "workflow-state.json").write_text(
            '{"stage": "check", "status": "awaiting_user_confirmation"}\n',
            encoding="utf-8",
        )
        (task_dir / "finish-work-checklist.md").write_text("ok\n", encoding="utf-8")
        workflow_dir = root / ".trellis" / "scripts" / "workflow"
    ...
    def test_patch_task_store_archive_guard_allows_review_gate_stage_after_validate_passes(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="task-store-archive-review-gate-stage-"))
        self.addCleanup(shutil.rmtree, root)
        package_dir = root / "pkg"
        package_dir.mkdir(parents=True, exist_ok=True)
        (package_dir / "__init__.py").write_text("", encoding="utf-8")
        ...
        (task_dir / "task.json").write_text('{"status": "in_progress"}\n', encoding="utf-8")
        (task_dir / "workflow-state.json").write_text(
            '{"stage": "review-gate", "status": "awaiting_user_confirmation"}\n',
            encoding="utf-8",
        )
        (task_dir / "finish-work-checklist.md").write_text("ok\n", encoding="utf-8")
        workflow_dir = root / ".trellis" / "scripts" / "workflow"
    ...
    def test_patch_task_store_archive_guard_allows_delivery_stage_after_validate_passes_with_exit_ready_status(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="task-store-archive-delivery-stage-"))
        self.addCleanup(shutil.rmtree, root)
        package_dir = root / "pkg"
        package_dir.mkdir(parents=True, exist_ok=True)
        (package_dir / "__init__.py").write_text("", encoding="utf-8")
        ...
        (task_dir / "task.json").write_text('{"status": "in_progress"}\n', encoding="utf-8")
        (task_dir / "workflow-state.json").write_text(
            '{"stage": "delivery", "status": "awaiting_user_confirmation"}\n',
            encoding="utf-8",
        )
        (task_dir / "finish-work-checklist.md").write_text("ok\n", encoding="utf-8")
        workflow_dir = root / ".trellis" / "scripts" / "workflow"
    ...
    def test_patch_task_store_archive_guard_blocks_check_stage_without_exit_ready_status(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="task-store-archive-check-stage-blocked-"))
        self.addCleanup(shutil.rmtree, root)
        package_dir = root / "pkg"
        package_dir.mkdir(parents=True, exist_ok=True)
        (package_dir / "__init__.py").write_text("", encoding="utf-8")
        ...
        (task_dir / "task.json").write_text('{"status": "in_progress"}\n', encoding="utf-8")
        (task_dir / "workflow-state.json").write_text(
            '{"stage": "check", "status": "in_progress"}\n',
            encoding="utf-8",
        )
        (task_dir / "finish-work-checklist.md").write_text("ok\n", encoding="utf-8")
        workflow_dir = root / ".trellis" / "scripts" / "workflow"
        ...
        with redirect_stderr(stderr):
            result = runtime_module.cmd_archive(object())
        self.assertEqual(result, 1)
        self.assertIn("current workflow-state status", stderr.getvalue())

    def test_patch_task_store_archive_guard_blocks_review_gate_stage_without_exit_ready_status(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="task-store-archive-review-gate-stage-blocked-"))
        self.addCleanup(shutil.rmtree, root)
        package_dir = root / "pkg"
        package_dir.mkdir(parents=True, exist_ok=True)
        (package_dir / "__init__.py").write_text("", encoding="utf-8")
        ...
        (task_dir / "task.json").write_text('{"status": "in_progress"}\n', encoding="utf-8")
        (task_dir / "workflow-state.json").write_text(
            '{"stage": "review-gate", "status": "blocked"}\n',
            encoding="utf-8",
        )
        (task_dir / "finish-work-checklist.md").write_text("ok\n", encoding="utf-8")
        workflow_dir = root / ".trellis" / "scripts" / "workflow"
