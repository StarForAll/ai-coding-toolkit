...
`task.json` records task metadata. Common fields:
...
For monorepo projects with multiple packages, prefix with the package name:

```jsonl
{"file": ".trellis/spec/<package>/backend/index.md", "reason": "Package backend conventions"}
```

Replace `<package>` with the actual package name from `.trellis/config.yaml`.
...
| Change stage routing | `.trellis/workflow.md`, `workflow-state.py`, and the stage transition protocol. |
