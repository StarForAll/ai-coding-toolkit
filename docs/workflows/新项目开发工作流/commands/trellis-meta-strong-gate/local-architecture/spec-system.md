...
Replace `<package-a>`, `<package-b>` with the actual package names from `.trellis/config.yaml`.
...
{"file": ".trellis/spec/backend/test-conventions.md", "reason": "Test expectations"}
```

Monorepo example (prefix with package name):
