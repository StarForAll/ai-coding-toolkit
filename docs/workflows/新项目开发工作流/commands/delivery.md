...
- `project_engagement_type` 必须明确为 `external_outsourcing`
- `kickoff_payment_ratio` 与 `kickoff_payment_received` 必须与此前开工记录一致；不得在交付阶段回补或改写
- 对客沟通中应友善说明：尾款确认前不能完成最终源码/控制权移交
