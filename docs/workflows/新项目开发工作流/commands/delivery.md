...
|---------|---------------------------|----------------|------|
| 全部通过，准备收尾 | `/trellis:record-session` | 进入会话收尾，或显式触发 `record-session` skill | **默认推荐**。在 `/trellis:record-session` 阶段按 Trellis 原生顺序先 archive，再通过 `add_session.py` 完成记录与元数据闭环 |
