...
此验证覆盖 `assessment.md` 双轨字段完整性、`task_plan.md` 移交任务结构、以及 `delivery/` 交付事件文档。若验证失败，不得进入正式交付。
...
|---------|---------------------------|----------------|------|
| 全部通过，准备收尾 | `/trellis:record-session` | 进入会话收尾，或显式触发 `record-session` skill | **默认推荐**。先通过 helper 完成记录与元数据闭环，再 archive |
