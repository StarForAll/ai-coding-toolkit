---
name: delivery
description: 开发完成？准备交付 — 验收测试、交付物生成、变更日志、经验沉淀。触发词：准备交付、跑验收、整理交付物、项目收尾、做个流程复盘、记录这次踩坑
---

# /trellis:delivery — 项目测试、交付与沉淀

> **Workflow Position**: §6+§7 → 前: `/trellis:finish-work` → 后: `/trellis:record-session`
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:delivery`） · ✅ Cursor（命令名: delivery） · ✅ OpenCode（TUI: `/trellis:delivery`；CLI: `trellis/delivery`；见 `opencode/README.md`） · ⚠️ Codex（通过 AGENTS.md NL 路由触发，不提供项目级 `/trellis:delivery` 命令；见 `codex/README.md`） · ⚠️ Gemini（兼容层；见 `gemini/README.md`）
...
| 场景 | 调用能力 | 调用级别 | 说明 |
|------|---------|---------|------|
| 项目依赖/版本确认 | `Context7` | 默认 | 获取最新框架文档。若无法获取，标记 `[Evidence Gap]`，仅引用项目内已有版本与用法证据 |
| 依赖安全漏洞检查 | `exa_search` | 按需 | 搜索已知漏洞信息。仅在存在依赖升级或上线前风险确认需求时触发 |

**调用 Skill**：使用 Skill 工具执行 `verification-before-completion`。若该 skill 不可用，手动记录测试/类型检查/lint 的实际命令、输出摘要与结论。
...
**调用 Skill**：使用 Skill 工具执行 `doc-coauthoring`，协同撰写交付文档。若该 skill 不可用，手动按“客户交付物 / 开发交付物 / 验收证据”三段结构整理。
...
**调用 Skill**：使用 Skill 工具执行 `changelog-generator`，从 git commits 自动生成变更日志。若该 skill 不可用，手动按功能、修复、风险、迁移说明四类整理。

### Step 8: 代码审查

**调用 Skill**：使用 Skill 工具执行 `requesting-code-review`，完成 PR 前审查清单。若该 skill 不可用，手动列出审查范围、验证证据和剩余风险。
