...
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:delivery`） · ✅ Cursor（命令名: delivery） · ✅ OpenCode（TUI: `/trellis:delivery`；CLI: `trellis/delivery`；见 `opencode/README.md`） · ⚠️ Codex（同项目会安装 `delivery` skill；通过自然语言或显式 skill 触发，不提供项目级 `/trellis:delivery` 命令；见 `codex/README.md`） · ⚠️ Gemini（兼容层；见 `gemini/README.md`）
...
| 场景 | 调用能力 | 说明 |
|------|---------|------|
| 项目依赖/版本确认 | `Context7` | 获取最新框架文档 |
| 依赖安全漏洞检查 | `exa_search` | 搜索已知漏洞信息 |
