...
在创建或复用 `brainstorm` task 前，先检查 `feasibility` 是否已完成且 `assessment.md` 仍有效。
...
- `task_dir/prd.md` 必须包含：
  ...
  - `估算置信度`
  - `估算前提`
- `docs/requirements/customer-facing-prd.md` 必须包含：
  - `## 项目级粗估摘要`
  - `预计总工期`
  - `预计完工窗口`
  - `估算说明`
- 若暂时无法给出单点值，也必须给出**区间估算**与适用前提；“后面再说”“先不估”都不算通过
- 当前 workflow 通过 `workflow-state.py validate` 把这条门禁卡在 `design` 及后续阶段入口上；其中：
  - `task_dir/prd.md` 的项目级粗估会在 `feasibility/brainstorm` 之后的全部后续阶段持续校验
  - `customer-facing-prd.md` 的存在与粗估摘要从 `design/plan` 开始强制
  - `L0` 直进 `start` / `test-first` 时则只强制 `task_dir/prd.md` 里的项目级粗估
