...
- 若下一步准备进入 `design`，则必须先明确：`design -> 3.7 技术架构确认后的项目 Spec 对齐` 阶段会把 `sonar-scanner` 纳入当前项目的固定质量门禁；该门禁不因语言不同而省略

### Step 8.5: 固定生成项目级 `project-audit` 任务（强制）

在本轮需求确认后，必须把一个固定的项目级终局任务写入当前工作底稿，并在后续 `plan` 阶段落入 `task_plan.md`：

- 任务名：`PROJECT-AUDIT`
- 任务域：`项目级审查`
- 作用：在**全部 `代码相关` 任务完成后**，对项目所有代码内容做一次统一的全局代码审查与查缺补漏
- 该任务不是每个子任务的后置动作，而是项目级唯一一次终局审查任务
...
| 需求已准确，且准备先冻结设计 | `/trellis:design` | 前提：项目级双需求文档已生成并同步；若当前阶段是首次入口则已完成 `main` 分支门禁；并且已明确 `sonar-scanner` 会作为固定质量门禁进入 `design -> 3.7` |
...
- 已明确 `sonar-scanner` 会在 `design -> 3.7 技术架构确认后的项目 Spec 对齐` 阶段作为固定质量门禁写入项目自动化检查矩阵
...
| 做设计 | `/trellis:design` | 进入设计阶段，或显式触发 `design` skill | 前提：项目级双需求文档已生成并同步；若当前阶段是首次入口则已完成 `main` 分支门禁；并且进入 `design` 时必须把 `sonar-scanner` 作为固定质量门禁写入项目自动化检查矩阵 |
