...
当需求已达到”可确认”状态后，在进入 `design` / `plan` 前，必须先补齐目标项目中的两份正式需求文档：

- `docs/requirements/customer-facing-prd.md`
- `docs/requirements/developer-facing-prd.md`

**L0 降级**：L0 单任务闭环可不强制生成双 PRD，但 `prd.md` 工作底稿仍需覆盖目标、范围、验收、边界四项。进入 `start` 前至少确认需求基线已就绪。
...
- 后续若命中 [需求变更管理执行卡](../需求变更管理执行卡.md) 且变更获批，必须同步更新这两份文档
...
| **Standard** | L1 / 多文件任务 | 完整双 PRD | brainstorm → design → plan → start（自动 before-dev）→ check → finish-work → delivery → record-session |
| **Strict** | L2 / 外包 / 新客户 / 高风险 / 跨层 | 完整双 PRD + feasibility + design 3.7 + 交付控制 + project-audit（项目级） + review-gate（任务级条件触发） | 保留当前严格全链路；实施前仍由 start 自动触发 before-dev |
...
| 需求已准确，且准备先冻结设计 | `/trellis:design` | 前提：项目级双需求文档已生成并同步；若当前阶段是首次入口则已完成 `main` 分支门禁；进入 `design` 时需明确项目自动化检查矩阵（采用 Sonar 的项目必须写真实命令，未采用时必须写替代门禁和原因） |
