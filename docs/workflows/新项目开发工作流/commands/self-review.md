...
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:self-review`） · ✅ Cursor（命令名: self-review） · ✅ OpenCode（TUI: `/trellis:self-review`；CLI: `trellis/self-review`；见 `opencode/README.md`） · ⚠️ Codex（同项目会安装 `self-review` skill；通过自然语言或显式 skill 触发，不提供项目级 `/trellis:self-review` 命令；见 `codex/README.md`） · ⚠️ Gemini（兼容层；见 `gemini/README.md`）
...
| 场景 | 调用能力 | 说明 |
|------|---------|------|
| 代码影响面分析 | `ace.search_context` | 查找相似代码、调用关系 |
| 复杂验证逻辑推理 | `sequential-thinking` | 多步推理、分支探索 |
...
| 偏差来自冻结后新增 / 修改 / 删除需求 | `§2.5 需求变更管理` | 先完成评估与确认；获批后再回到受影响的最早阶段 |
| 偏差仅是纯澄清 | 留在当前阶段 | 仅限不改变范围、接口契约、验收标准、成本、工期 |
