...
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:self-review`） · ✅ Cursor（命令名: self-review） · ✅ OpenCode（TUI: `/trellis:self-review`；CLI: `trellis/self-review`；见 `opencode/README.md`） · ⚠️ Codex（通过 AGENTS.md NL 路由触发，不提供项目级 `/trellis:self-review` 命令；见 `codex/README.md`） · ⚠️ Gemini（兼容层；见 `gemini/README.md`）
...
| 复杂验证逻辑推理 | `sequential-thinking` | 按需 | 当验证链路涉及 ≥3 个条件组合、异常分支或交叉影响时触发 |

**调用 Skill**：使用 Skill 工具执行 `verification-before-completion`，坚持“证据先于断言”。若该 skill 不可用，至少明确列出验证命令、关键输出和 `pass / fail / not run` 结论。
...
**安全检查**：按需使用 Skill 工具执行 `sharp-edges`，识别危险 API 和配置。若该 skill 不可用，手动检查 fail-open 默认值、危险配置和易误用接口。
