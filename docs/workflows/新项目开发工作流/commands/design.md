...
`workflow-state.py validate` 在这里还会额外校验：

- `task_dir/prd.md` 是否已补齐项目级粗估
- `docs/requirements/customer-facing-prd.md` 是否已补齐项目级粗估摘要

任一缺失都表示 `brainstorm` 尚未真正收口，不允许进入 design。
