...
> 若 `PRD` 已冻结后，用户在本阶段提出新增 / 修改 / 删除需求，不在当前阶段直接吸收，先执行 `§2.5 需求变更管理`；只有纯澄清且不改变范围、接口契约、验收标准、成本、工期时，才留在本阶段直接处理。
...
| 冻结后出现新增 / 修改 / 删除需求 | `§2.5 需求变更管理` | 先做影响评估、成本/工期确认与审批；获批后再回到受影响的最早阶段 |
