...
get_context.py 输出
    ...
    │   └── 进入 [需求变更管理执行卡](../需求变更管理执行卡.md)（先做影响评估、成本/工期确认与审批；完成后回到受影响的最早阶段）
    │
    ...
    │
    ├── 质量检查完成
    │   └── 路由 → /trellis:finish-work（默认）/ /trellis:review-gate（条件触发：安全/跨层/高 blast radius/外包/L2）
    │
    └── 用户要求继续/跳到某阶段
        └── 直接跳转到指定命令
```
...
| PRD 已冻结后说 "这个功能再加一个" "这个页面流程改一下" "这个接口字段要调整" "这个验收标准我想改" "这个功能先不做了" | [需求变更管理执行卡](../需求变更管理执行卡.md)（全局分支，不在当前阶段直接吸收） |
...
- 只要属于新增 / 修改 / 删除，就进入 [需求变更管理执行卡](../需求变更管理执行卡.md)
- 这里的 [需求变更管理执行卡](../需求变更管理执行卡.md) 是**全局流程分支**，不是独立 slash command
