...
| project-audit → review-gate | `workflow-state.py set <dir> --stage-status awaiting_user_confirmation --awaiting-user-confirmation true` | `workflow-state.py set <dir> --stage review-gate --stage-status in_progress --awaiting-user-confirmation false --allowed-next project-audit,delivery,implementation --transition-from project-audit`（仅当 `project_audit_code_changes = no` 时允许；只要本轮改过代码，必须先回 `check`。无论去向是否为 delivery，离开 `project-audit` 前都必须先写出结构化 `project_audit_gate_status`） |
