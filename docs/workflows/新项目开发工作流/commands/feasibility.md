...
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:feasibility`） · ✅ Cursor（命令名: feasibility） · ✅ OpenCode（TUI: `/trellis:feasibility`；CLI: `trellis/feasibility`；见 `opencode/README.md`） · ⚠️ Codex（通过 AGENTS.md NL 路由触发，不提供项目级 `/trellis:feasibility` 命令；见 `codex/README.md`） · ⚠️ Gemini（兼容层；见 `gemini/README.md`）
>
...
| 场景 | 调用能力 | 调用级别 | 说明 |
|------|---------|---------|------|
| 竞品分析、技术方案深度调研 | `exa_create_research` | 按需 | 回退：`grok-search`。无法联网时不输出行业最新情况结论，改为本地已知风险清单，标记 `[Evidence Gap]` |
| 风险评估复杂推理 | `sequential-thinking` | 按需 | 当风险评估涉及 ≥3 个决策分支或推理步骤 >3 步时触发 |
| 参考 GitHub 开源项目 | `deepwiki` | 按需 | 回退：`exa_search` |
...
2. **调用 Skill**：使用 Skill 工具执行 `demand-risk-assessment` skill
