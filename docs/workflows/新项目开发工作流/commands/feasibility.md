...
- 这里所说的 personal 首次入口“基线”只覆盖离开 `brainstorm` 前必须冻结的最低 assessment 字段，不等同于本命令可继续产出的完整 feasibility 评估内容（如风险分析、商务预判、外部交付控制字段等）。
...
- personal 项目若仅在 `brainstorm` 内补齐最低 baseline，后续不强制回到 feasibility；只有当项目后来需要完整 feasibility 决策链所覆盖的结论或字段时，才应回到本命令补齐。
