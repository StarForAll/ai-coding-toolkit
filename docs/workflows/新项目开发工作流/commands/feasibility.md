...
- 当前 workflow 安装器会执行同口径校验；若首次入口不是本命令而是 `brainstorm`，则应在进入 `brainstorm` 前完成同口径判断
...
|---------|---------------------------|----------------|------|
| 继续推进项目 | `/trellis:brainstorm` | 继续需求发现，或显式触发 `brainstorm` skill | **默认推荐**。仅在用户明确确认后才允许切换 |
...
| 需求已经很明确，跳过 brainstorm | `/trellis:design` | 直接进入设计，或显式触发 `design` skill | 仅在用户明确确认后才允许；且仍需先满足 design 的正式输入门禁 |
| 需求简单，直接写代码 | `/trellis:start` | 直接进入实施，或显式触发 `start` skill | 仅在用户明确确认后才允许；适合小改动 |
