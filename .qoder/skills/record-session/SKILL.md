...
Codex-specific helper / recovery path. Qoder's default `finish-work` entrypoint
continues to use `add_session.py`; use the helper examples below only when you
explicitly need helper / resume semantics.
...
If you only need Qoder's generic manual record path and do not need the
...
| `python3 ./.trellis/scripts/add_session.py --title "..." --commit "..."` | Generic Qoder manual session record path |
