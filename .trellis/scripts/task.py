...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    def _print_task(dir_name: str, indent: int = 0) -> None:
        ...
        if filter_mine:
            print(f"{prefix}{dir_name}/ ({task.status}){pkg_tag}{progress}{marker}")
        else:
            print(
                f"{prefix}{dir_name}/ ({task.status}){pkg_tag}{progress} "
...
def cmd_list_archive(args: argparse.Namespace) -> int:
    ...
    if month:
        ...
        if month_dir.is_dir():
            print(f"[{month}]")
            for directory in sorted(month_dir.iterdir()):
                if directory.is_dir():
        ...
        for month_dir in sorted(archive_dir.iterdir()):
            if month_dir.is_dir():
                ...
                count = sum(1 for directory in month_dir.iterdir() if directory.is_dir())
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "init-context":
        ...
        print(
            "sub-agent-capable platforms and curated by the AI during Phase 1.2.",
            file=sys.stderr,
        )
        print("See .trellis/workflow.md Phase 1.2 or run:", file=sys.stderr)
        print(
            "  python3 ./.trellis/scripts/get_context.py --mode phase --step 1.2",
            file=sys.stderr,
        )
        print(
