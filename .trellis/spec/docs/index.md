...
- **multi-cli reviewer command pairing**: "需要调用 `multi-cli-review` 时，由当前 CLI 生成至少两条只在 `--reviewer-id` 上不同的 reviewer 命令，并同时生成匹配的 `multi-cli-review-action` 聚合命令；task-level `reviewer-id` 默认使用 `a/b/c/d` 这类字母槽位，真实 CLI 身份记录在 `source-cli`" — appears in review-gate, project-audit, 总纲, walkthrough, platform instructions, and any reviewer command pack templates
