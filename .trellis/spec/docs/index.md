...
- **workflow close-out order**: for the installed target-project workflow, fresh-baseline close-out enters through `finish-work`, then runs `record-session-helper.py`, and only then runs `task.py archive`. This is a product/runtime contract for workflow-installed target projects. Do not confuse it with this source repository's own repo-local `trellis-finish-work` skill, which is a maintainer-side closure helper with a different internal sequencing contract.
