...
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:check`） · ✅ OpenCode（TUI: `/trellis:check`；CLI: `trellis/check`；OpenCode 入口见目标项目 AGENTS.md 路由表） · ⚠️ Codex（通过 AGENTS.md NL 路由触发，不提供项目级 `/trellis:check` 命令；Codex 入口见目标项目 AGENTS.md 路由表）

> **Strong Gate**: 本阶段受 阶段状态机与强门禁协议 约束。`check` 完成后不能自动进入 `review-gate`、`delivery` 或 native `finish-work`，必须先等待用户确认。
...
| 偏差来自冻结后新增 / 修改 / 删除需求 | [需求变更管理执行卡](../../../.trellis/workflow-docs/需求变更管理执行卡.md) | 同上 | 先完成评估与确认；用户接受并入当前轮次后再回到受影响的最早阶段 |
