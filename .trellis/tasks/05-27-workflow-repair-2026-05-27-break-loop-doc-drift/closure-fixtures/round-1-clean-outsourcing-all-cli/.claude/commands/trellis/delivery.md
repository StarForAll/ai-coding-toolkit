...
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:delivery`） · ✅ OpenCode（TUI: `/trellis:delivery`；CLI: `trellis/delivery`；OpenCode 入口见目标项目 AGENTS.md 路由表） · ⚠️ Codex（通过 AGENTS.md NL 路由触发，不提供项目级 `/trellis:delivery` 命令；Codex 入口见目标项目 AGENTS.md 路由表）

> **Strong Gate**: 本阶段受 阶段状态机与强门禁协议 约束。`delivery` 是项目级/交付级验收与交付阶段，不等同于单任务级 `finish-work`。若当前轮还需要对**当前活动任务**做原生 Trellis 收尾，再进入 `/trellis:finish-work`。
...
- `task_level_check_task` 在新文档里应填写真实 task dir name 或 `.trellis/tasks/<task-dir-name>`；`current_active_task` / `self` / `parent` 仅保留旧数据兼容读取，不再是推荐写法
- 这条“显式绑定任务级 owner”的要求只用于把任务级 `check` 证据接到项目级 `delivery` 上；若 formal `PROJECT-AUDIT` 之后还要回到任务级 `check` / `review-gate`，则必须先切回 `task_level_check_task` 指向的任务级 owner，不能在项目级 carrier 上直接进入任务级阶段
...
> 若验收或交付阶段命中需求讨论，按 [需求变更管理执行卡](../../../.trellis/workflow-docs/需求变更管理执行卡.md) 分流：纯澄清留在当前阶段；新增 / 修改 / 删除进入变更管理，不直接混入本轮交付。
...
```bash
python3 .trellis/scripts/workflow/delivery-control-validate.py --phase delivery --task-dir <task-dir>
```
...
python3 .trellis/scripts/workflow/source-watermark-guard.py --task-dir <task-dir> --mode check
```
...
```bash
python3 .trellis/scripts/workflow/source-watermark-guard.py --task-dir <task-dir> --mode repair
python3 .trellis/scripts/workflow/source-watermark-guard.py --task-dir <task-dir> --mode check
```

设计边界、术语与证据链口径以 [源码水印与归属证据链执行卡](../../../.trellis/workflow-docs/源码水印与归属证据链执行卡.md) 为准。
...
```bash
python3 .trellis/scripts/workflow/workflow-state.py set <task-dir> \
...
| 验收中出现冻结后新增 / 修改 / 删除需求 | [需求变更管理执行卡](../../../.trellis/workflow-docs/需求变更管理执行卡.md) | 同上 | 先完成变更评估与确认；不要直接混入当前交付 |
