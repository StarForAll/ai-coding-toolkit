...
Original OpenCode baseline continue command for fixture testing.
...
```bash
python3 .trellis/scripts/workflow/workflow-state.py route <task-dir> --project-root <project-root>
```
...
| action | 含义 | 执行动作 |
|--------|------|---------|
| `entry_choice_required` | 当前 session 尚无 active task，且项目中也没有可继续任务 | 先判断当前意图。如果是 workflow / 项目只读分析、元审计或 A/A+ 纯分析，则保持 `no_task` 直接分析，不创建任务；如果是开始新的实现任务，再进入 `target` 对应的正式入口。若项目还没有可复用的有效 assessment，则所有项目默认入口都应先走 `/trellis:feasibility`；只有在 route 明确复用了现有 assessment 且允许继续 brainstorm 时，才进入 `/trellis:brainstorm`。若 route 同时给出 `profile_hint=unknown`，保持 `feasibility` 的保守回退，并先**直接询问用户当前项目应按 outsourcing 还是 personal 处理**；不要尝试从聊天语气、仓库名或零散文档自行猜测，也不要跳过 feasibility。不要期待存在公开的 `/trellis:implementation` 入口。 |
...
| `repair_needed` | 状态文件缺失或损坏 | 先不带 `--apply` 运行 `workflow-state.py repair`，查看它输出的 `status`、`evidence`、`blockers`。若输出 `repair_ready`，在用户确认后再带 `--apply` 重建；若输出 `manual_confirmation_required`，必须先让用户明确当前已确认阶段以及缺失的执行边界信息，再补齐参数后重试。非执行阶段通常只需 `--stage <stage>`；若当前阶段是 `implementation`，还必须显式补齐 `--execution-authorized true` 与 `--transition-from <上一阶段>`，不能靠任务产物反推。成功 apply 后，还应重新运行 `route` 或 `validate`，确认恢复完成再继续。 |
