---
name: design
description: 需求冻结了？开始设计 — 先核对设计输入，再做技术选型研究，用户确认技术架构后分块落盘设计文档、项目文档和工程化联动。适用场景提示：开始设计、画架构图、技术选型、设计方案、架构设计、技术方案、选型、接口设计
---

# /trellis:design — 强门禁设计阶段

> **Workflow Position**: §3 → 前: `/trellis:brainstorm` → 后: `/trellis:plan`
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:design`） · ✅ OpenCode（TUI: `/trellis:design`；CLI: `trellis/design`；OpenCode 入口见目标项目 AGENTS.md 路由表） · ⚠️ Codex（通过 AGENTS.md NL 路由触发，不提供项目级 `/trellis:design` 命令；Codex 入口见目标项目 AGENTS.md 路由表）
>
> **Gate Rule**: 当前阶段受 阶段状态机与强门禁协议 约束。设计阶段只能在当前已确认阶段内重入，不能因为“设计文档看起来差不多齐了”就自动进入 `/trellis:plan`。
...
> 若 `PRD` 已冻结后命中需求讨论，按 [需求变更管理执行卡](../../../.trellis/workflow-docs/需求变更管理执行卡.md) 分流：纯澄清留在当前阶段；新增 / 修改 / 删除进入变更管理，不直接在本阶段吸收。

---

## 强门禁规则

当前阶段受 阶段状态机与强门禁协议 约束。

1. 技术架构确认前，只有产品信息和功能需求允许同步到目标项目正式文档
2. 技术架构确认后，design 后半段仍需按子块执行；每完成一个子块，都必须停下来给用户确认后才能继续
3. `workflow-state.json.current_block` 只允许使用 `input-review`、`option-research`、`A`、`B`、`C`、`D` 这几个 design 固定子块标识；它用于跟踪当前 design 子块，而不是自由备注字段
...
- 推荐外部操作顺序固定为：
  ...
  3. 再去 [Stitch](https://stitch.withgoogle.com/) 按页面 / 流程逐个生成首版 UI 原型
  4. 最后进入 Figma 做现代视觉风格参考与校正，再回收结构化结论
- **执行边界（强制）**：
...
3. 在当前 CLI 中生成 `design/STITCH-PROMPT.md` 草稿；该文件同时承担 Stitch `DESIGN.md` 的设计系统语义
4. 在 `design/STITCH-PROMPT.md` 中固定：
   - UI 界面文案默认使用中文
   - 给 Stitch 的执行 prompt 默认使用英文
   - 去 AI 味全局禁止项
5. 按页面 / 流程从 `design/STITCH-PROMPT.md` 中提取英文执行 Prompt，带到 `https://stitch.withgoogle.com/` 逐个生成首版原型
6. 基于 Stitch 首版原型进入 Figma 做现代视觉风格参考与校正；Figma 只作为整体风格参考，不作为具体内容布局照抄依据
7. 把确认后的页面结构、组件清单、交互要点、视觉方向记录回 task 工作底稿
...
- Figma 只承担整体视觉风格参考与校正职责，不承担页面内容布局的权威来源；页面结构仍以已确认的 Stitch 首版原型与回收后的结构化设计结论为准
- 以 阶段状态机与强门禁协议 中的 design 参考资产隔离规则为准
...
- 条件文档：
  ...
  - `design/IDD.md`
  - `design/AID.md`
  - `design/STITCH-PROMPT.md`                           （同时承担 Stitch `DESIGN.md` 的设计系统语义）
- 按需：
...
1. 根据技术架构，使用 `/ops/projects/personal/ai-coding-toolkit/trellis-library/cli.py assemble` 选择并导入合适 spec 到当前项目 `.trellis/spec/`
...
- 混合型 spec 按“被第三方官方文档直接约束的部分”与“项目私有封装部分”拆开判断；例如基于 React 的内部组件库：React API / 生命周期 / 官方约束部分需要 `Context7`，内部封装命名、团队约定和项目私有包装层不强制 `Context7`
- 错漏分析至少覆盖：
  - API / 配置过时
  - 能力假设错误
  - 缺少必要约束
...
- 设计边界与术语以 [源码水印与归属证据链执行卡](../../../.trellis/workflow-docs/源码水印与归属证据链执行卡.md) 为准
...
```bash
python3 .trellis/scripts/workflow/design-export.py --validate <design-dir>
```
...
| 冻结后出现新增 / 修改 / 删除需求 | [需求变更管理执行卡](../../../.trellis/workflow-docs/需求变更管理执行卡.md) | 同上 | 不直接在当前阶段吸收 |
