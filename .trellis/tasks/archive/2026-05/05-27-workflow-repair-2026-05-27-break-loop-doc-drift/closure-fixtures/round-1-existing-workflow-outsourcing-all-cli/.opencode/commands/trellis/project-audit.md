...
> **Cross-CLI**: ✅ Claude Code（项目命令：`/trellis:project-audit`） · ✅ OpenCode（TUI: `/trellis:project-audit`；CLI: `trellis/project-audit`；OpenCode 入口见目标项目 AGENTS.md 路由表） · ⚠️ Codex（通过 AGENTS.md NL 路由触发，不提供项目级 `/trellis:project-audit` 命令；Codex 入口见目标项目 AGENTS.md 路由表）

> **Strong Gate**: 本阶段受 阶段状态机与强门禁协议 约束。project-audit 完成后，必须等待用户明确确认，不能自动切到 `check` / `finish-work` / `delivery`。
...
| 审查发现冻结后新增 / 修改 / 删除需求 | [需求变更管理执行卡](../../../.trellis/workflow-docs/需求变更管理执行卡.md) | 同上 | 先完成变更评估，不直接混入本轮审查修改 |
