# workflow-repair-2026-05-27-create-pr-reference
...
1. WS-001: embedded `.trellis/workflow.md` still advertises removed
   `task.py create-pr`
2. WS-002: OpenCode `inject-subagent-context.js` still parses legacy route text
   instead of current JSON route output
3. WS-003: Claude/OpenCode `trellis-update-spec` skills mention
   `/trellis:update-spec` and `/trellis:break-loop` commands that are not
   installed
4. WS-004: OpenCode hook-customization docs still claim
   `.opencode/package.json` exposes plugin registration that the installed file
   does not contain
