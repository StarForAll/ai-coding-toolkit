...
| Change the wrap-up flow | Update Phase 3 and `finish-work` related descriptions. Note the current split: Phase 3.1 = AI-driven code commits (batched, user-confirmed), Phase 3.2 = `/finish-work` (archive + record session). `/finish-work` refuses to run if the working tree is dirty. |
