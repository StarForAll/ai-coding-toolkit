...
Verify that `workflow-audit` stops cleanly when a task-based mode is required but the `brainstorm` entrypoint is unavailable.

## Input

User input:

> Run a full structured workflow audit for `docs/workflows/新项目开发工作流/` with task tracking, but the current environment does not have the `brainstorm` skill available.
...
- detect that the required `brainstorm` entrypoint is unavailable before entering task context
