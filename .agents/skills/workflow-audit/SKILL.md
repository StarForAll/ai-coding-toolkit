...
- Lightweight mode:
  reuse `brainstorm` discipline for evidence-first work, action-before-asking, and one-question-at-a-time clarification, but do not create a task, do not create `prd.md`, and do not create `audit-report.md`
- Non-trivial mode:
  explicitly enter the `brainstorm` mainline
...
Output:
Enter the non-trivial path, create an audit task, use the `brainstorm` mainline, emit a Codex handoff block when required, and maintain `audit-report.md` inside the task.
