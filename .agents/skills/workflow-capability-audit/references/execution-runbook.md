...
Same-version explicit continue example:
...
--record-correction "Updated workflow source for Trellis compatibility alignment." \
...
- full audit is allowed whenever `current > compatible`
- equal version stops by default unless `--allow-equal-version-continue` is passed
- older version aborts with a non-zero contract-violation error because the workflow contract requires `COMPATIBLE_TRELLIS_VERSION <= trellis -v`
