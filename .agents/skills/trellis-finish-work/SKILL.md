...
description: "Wrap up the current session: archive completed tasks and record session progress to the developer journal. Code commits are NOT done here — they must be completed in Phase 3.1 before invoking this command. Use when code is already committed and ready to finalize the session."
---
...
Filter out paths under `.trellis/workspace/` and `.trellis/tasks/` — those are managed by `record-session-helper.py` and `task.py archive` auto-commits and will appear dirty as part of this skill's own work.
...
- **Any remaining path looks like current-task work** — bail out with:
  > "Working tree has uncommitted code changes from this task: `<list>`. Return to workflow Phase 3.1 to commit them before running ``finish-work` (Trellis command)`."
