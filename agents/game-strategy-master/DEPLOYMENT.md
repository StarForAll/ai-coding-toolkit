...
这份文档说明如何把 `agents/game-strategy-master/` 里的源资产，适配到目标项目中的
...
- 目标路径：目标项目的 `.claude/agents/game-strategy-master.md`
...
```markdown
---
name: game-strategy-master
description: |
  Mobile game strategy specialist for evidence-bounded strategy diagnosis,
  system synthesis, leverage-point discovery, conflict trade-offs, and version
...
- 目标路径：目标项目的 `.opencode/agents/game-strategy-master.md`
- 文件格式：Markdown + YAML frontmatter
- agent 名默认来自文件名
...
- 目标路径：目标项目的 `.codex/agents/game-strategy-master.toml`
- 文件格式：TOML
...
```toml
name = "game-strategy-master"
description = "Mobile game strategy specialist for evidence-bounded strategy diagnosis, system synthesis, leverage-point discovery, conflict trade-offs, and version roadmap recommendations."
...
这个 agent 可以离线分析用户提供的策划案、测试反馈和指标摘要，但以下任务高度依赖
live evidence：
...
- Codex wrapper 只使用当前官方支持的自定义 agent 字段
