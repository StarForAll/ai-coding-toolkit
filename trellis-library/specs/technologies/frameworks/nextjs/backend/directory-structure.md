...
├── router.ts         # oRPC module router (procedure map)
├── lib/              # Core business logic (shared across procedures)
│   ├── client.ts     # External service clients
│   └── helpers.ts    # Helper functions
├── procedures/       # Individual oRPC procedure definitions
│   ├── create-item.ts
│   ├── update-item.ts
│   ├── delete-item.ts
│   └── list-items.ts
...
### `router.ts` - oRPC Module Router

The router exports a plain object mapping procedure names to procedure instances (not a Hono app).
...
These module routers are then composed in `packages/api/orpc/router.ts`. See [oRPC Usage](./orpc-usage.md) for the full main router setup.
...
|------|------------|---------|
| Procedures | Kebab-case verb-noun | `create-item.ts`, `list-items.ts`, `update-item.ts` |
...
| Type | Convention | Example |
|------|------------|---------|
| Schemas | `{action}{Entity}InputSchema` | `createItemInputSchema` |
| Output schemas | `{entity}Schema` | `itemSchema` |
| Types | PascalCase | `CreateItemInput` |
| Procedures | camelCase verb-noun | `createItem`, `listItems` |
| Module router | `{entity}Router` | `itemsRouter` |
